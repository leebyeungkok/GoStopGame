...
public class MainView extends SurfaceView implements SurfaceHolder.Callback{
    ...
    public void comBrainsendCardToField(int count, Card[] selectSendCard) {
        mChoiceNum = selectSendCard[0].mCardNum;
    ...
    public void comBrainGoStop() {
        mChoiceNum = STOP_NUM;	// 임의로 스톱한다.
        mHandler.sendMessage(Message.obtain(mHandler, H_GOSTOPCHOICE));
    }
    void comBrain() {
        sendCardToField(mInYourHandCardList[1]);
        return;
    }
    // 카드선택을 임으로 처리함.
    public void comBrainSendCardToField(int count, Card[] selectSendCard) {
        // 바닥에 넬 카드를 생각한다. 우선순위를 정한다.
        int tempChoiceNum = selectSendCard[0].mCardNum;
